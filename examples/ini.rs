...
mod tests {
    ...
    fn test_parse_section_header() {
        assert_eq!(IniParser::parse_section_header("[section]"), Some("section"));
        assert_eq!(IniParser::parse_section_header("[section with spaces]"), Some("section with spaces"));
    ...
    fn test_parse_property() {
        assert_eq!(IniParser::parse_property("key=value"), Some(("key", "value")));
        assert_eq!(IniParser::parse_property("key = value"), Some(("key", "value")));
        ...
        assert_eq!(IniParser::parse_property("multiple=equals=signs"), Some(("multiple", "equals=signs")));
    }
...
"#;
        ...
        assert_eq!(parser.get_global_property("global_prop"), Some(&"global_value".to_string()));

        let db_section = parser.get_section("database").unwrap();
        assert_eq!(db_section.get_property("host"), Some(&"localhost".to_string()));
        ...
        assert_eq!(cache_section.get_property("enabled"), Some(&"true".to_string()));
