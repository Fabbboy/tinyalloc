...
where
    M: Mapper + ?Sized,
{
    pub fn new(size: NonZeroUsize, mapper: &'mapper M) -> Result<Self, LargeError> {
        let self_size = core::mem::size_of::<Self>();
        let total_size = size
            .get()
            .checked_add(self_size)
            .ok_or(LargeError::SizeOverflow)?;
        let mut region = Region::new(mapper, NonZeroUsize::new(total_size).unwrap())
            .map_err(LargeError::MapError)?;
        region.activate().map_err(LargeError::MapError)?;
        let ptr = region.as_ptr();
        let user = unsafe { std::slice::from_raw_parts_mut(ptr.add(self_size), size.get()) };
        Ok(Self { region, user, link: Link::new() })
    }
}
