...
use tinyalloc_bitmap::{Bitmap, numeric::Bits};
    ...
    M: Mapper + ?Sized,
{
    pub fn new(size: usize, mapper: &'mapper M) -> Result<NonNull<Self>, ArenaError> {
        ...
        let aligned_rest = crate::config::align_slice(rest, core::mem::align_of::<usize>());
        ...
        let bitmap_storage = unsafe {
            core::slice::from_raw_parts_mut(bitmap_slice.as_mut_ptr() as *mut usize, bitmap_words)
        };
